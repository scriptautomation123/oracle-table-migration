-- ==================================================================
-- DATA LOAD: MYSCHEMA.IE_PC_SEQ_OUT -> MYSCHEMA.IE_PC_SEQ_OUT_NEW
-- ==================================================================
-- Generated: 2025-10-22 01:30:55
-- Estimated rows: 3,200,000
-- Estimated size: 12.50 GB
...
BEGIN
    EXECUTE IMMEDIATE 'SELECT COUNT(*) FROM MYSCHEMA.IE_PC_SEQ_OUT'
    INTO :v_source_count;
    
    DBMS_OUTPUT.PUT_LINE('Source table row count: ' || TO_CHAR(:v_source_count, '999,999,999,999'));
    DBMS_OUTPUT.PUT_LINE('');    IF :v_source_count != 3200000 THEN
        DBMS_OUTPUT.PUT_LINE('NOTE: Row count differs from discovery (3,200,000)');
...
DECLARE
    v_count NUMBER;
BEGIN
    SELECT COUNT(*) INTO v_count FROM MYSCHEMA.IE_PC_SEQ_OUT_NEW;
...
BEGIN
    FOR c IN (
        ...
        BEGIN
            EXECUTE IMMEDIATE 'ALTER TABLE MYSCHEMA.IE_PC_SEQ_OUT_NEW DISABLE CONSTRAINT ' || c.constraint_name;
...
PROMPT ================================================================
PROMPT This may take ~1.6 hours depending on system load
PROMPT Progress will be shown after completion
PROMPT ================================================================

-- Main data load with append hint
INSERT /*+ APPEND PARALLEL(2) NOLOGGING */ 
INTO MYSCHEMA.IE_PC_SEQ_OUT_NEW
(
    PROCESS_DATE, CREATE_DATE, SEQ_ID, BATCH_ID, SEQ_TYPE
)
SELECT /*+ PARALLEL(2) */
    PROCESS_DATE, CREATE_DATE, SEQ_ID, BATCH_ID, SEQ_TYPE
FROM 
    MYSCHEMA.IE_PC_SEQ_OUTORDER BY 
    PROCESS_DATE  -- Order by partition key for efficient loading;
...
BEGIN
    -- Get target count
    SELECT COUNT(*) INTO :v_target_count FROM MYSCHEMA.IE_PC_SEQ_OUT_NEW;
    ...
    DBMS_OUTPUT.PUT_LINE('Parallel Degree:    2');
    DBMS_OUTPUT.PUT_LINE('');
...
BEGIN
    FOR c IN (
        ...
        BEGIN
            EXECUTE IMMEDIATE 'ALTER TABLE MYSCHEMA.IE_PC_SEQ_OUT_NEW ENABLE NOVALIDATE CONSTRAINT ' || c.constraint_name;
            v_constraint_count := v_constraint_count + 1;
