...
import subprocess  # noqa: S404 - subprocess is needed for command execution
import time
...
class StepExecutor:
    ...
    def __init__(self, verbose: bool = False, sql_client: Optional[str] = None):
        """
        Initialize step executor

        Args:
            verbose: Enable verbose output
            sql_client: Force specific SQL client ('sqlcl' or 'sqlplus')
        """
        self.verbose = verbose
        self.sql_executor = SQLExecutor(explicit_client=sql_client, verbose=verbose)

    def execute_ddl_script(
        self, sql_file: Path, connection: str, output_file: Optional[Path] = None
    ) -> ExecutionResult:
        """
        Execute SQL file using auto-detected SQL client
        ...
        """
        sql_result = self.sql_executor.execute_sql_script(
            sql_file=sql_file, connection=connection, output_file=output_file
        )

        return ExecutionResult(
            success=sql_result.success,
            return_code=sql_result.return_code,
            stdout=sql_result.stdout,
            stderr=sql_result.stderr,
            duration_seconds=sql_result.execution_time_seconds,
            command=f"{sql_result.client_used} {connection} @{sql_file}",
        )
    ...
    def _execute_command(
        self, command: List[str], output_file: Optional[Path] = None
    ) -> ExecutionResult:
        """
        ...
        try:
            ...
            result = subprocess.run(
                command,
                shell=False,  # noqa: B602 - command is trusted (generated internally)
                stdout=stdout_fd,
