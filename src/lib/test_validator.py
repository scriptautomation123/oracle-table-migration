...
class TestValidator:
    ...
    def validate_discovery_config(self, config_file: Path) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                ...
                details={"table_count": len(tables), "tables": [t.get("name") for t in tables[:5]]}
            )
    ...
    def validate_generated_sql(self, output_dir: Path, expected_tables: int) -> ValidationResult:
        """
    ...
    def validate_sql_structure(self, sql_file: Path) -> ValidationResult:
        ...
        try:
            ...
            checks = {
                ...
                "has_create_or_alter": "CREATE" in content.upper() or "ALTER" in content.upper(),
    ...
    def validate_file_sizes(self, output_dir: Path, min_size_bytes: int = 100) -> ValidationResult:
        """
        ...
        if small_files:
            return ValidationResult(
                ...
                details={
                    "small_files": [(str(f), f.stat().st_size) for f in small_files[:10]]
                }
            )
        
        avg_size = sum(f.stat().st_size for f in sql_files) / len(sql_files) if sql_files else 0
    ...
    def validate_table_names(self, config_file: Path, sql_dirs: List[Path]) -> ValidationResult:
        """
