...
class TestReporter:
    ...
    def _create_markdown_report(self, results: Dict[str, Any]) -> str:
        ...
        lines.append(f"**Status:** {'✅ SUCCESS' if results.get('status') == 'SUCCESS' else '❌ FAILED'}\n")
        lines.append(f"**Duration:** {results.get('duration_seconds', 0):.2f} seconds\n")
    ...
    def create_summary_json(self, test_run_id: str, results: Dict[str, Any]) -> Dict[str, Any]:
        """
