#!/usr/bin/env python3
"""
Oracle Migration End-to-End Test Runner
=======================================
Comprehensive test/demo/deployment workflow following:
Schema → Models → Discovery → JSON → Generation → Templates → SQL

Modes:
- dev: Full workflow with validation
- test: Full workflow + cleanup on success
- prod: Full workflow + strict validation

Usage:
    python3 test/test_runner.py --connection "user/pass@host:port/service" --schema SCHEMA_NAME
"""
...
from test.lib.test_config import TestConfig
from test.lib.test_executor import StepExecutor
from test.lib.test_reporter import TestReporter
from test.lib.test_validator import TestValidator
...
class TestRunner:
    ...
    def step1_setup_schema(self):
        ...
        output_file = (
            self.output_dir / "00_schema_setup" / "comprehensive_oracle_ddl.log"
        )
        ...
        self.results["steps"]["schema_setup"] = {
            ...
            "message": (
                "Schema setup completed" if result.success else "Schema setup failed"
            ),
    ...
    def step2_generate_dataclasses(self):
        ...
        self.results["steps"]["dataclass_generation"] = {
            ...
            "message": (
                ...
                if result.success
                else "Dataclass generation failed"
            ),
            "details": {},
        }
        ...
        if not result.success:
            print(
                f"  ⚠ Warning: Dataclass generation had issues: {result.stderr[:200]}"
            )
        else:
    ...
    def step3_discover_schema(self):
        ...
        print(
            f"  ✓ Discovery complete: {tables_count} tables found ({result.duration_seconds:.2f}s)"
        )
    ...
    def step4_generate_ddl(self):
        ...
        print(
            f"  ✓ DDL generation complete: {sql_count} files ({result.duration_seconds:.2f}s)"
        )
    ...
    def step5_validate_generated(self):
        ...
        if not val_result.passed:
            raise RuntimeError(
                f"Directory structure validation failed: {val_result.message}"
            )
    ...
    def step6_execute_ddl(self):
        ...
        if executed < len(master_scripts):
            raise RuntimeError(
                f"Only {executed}/{len(master_scripts)} scripts executed successfully"
            )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Oracle Migration E2E Test Runner",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Development mode
  python3 test/test_runner.py --connection "system/oracle123@localhost:1521/FREEPDB1" --schema APP_DATA_OWNER
  
  # Test mode with cleanup
  python3 test/test_runner.py --connection "$ORACLE_CONN" --schema APP_DATA_OWNER --mode test
  ...
  python3 test/test_runner.py --skip-schema-setup
    ...
    parser.add_argument(
        "--connection",
        help="Oracle connection string (or set ORACLE_CONN environment variable)",
    )

    parser.add_argument(
        "--schema",
        help="Oracle schema name (or set ORACLE_SCHEMA environment variable)",
    )
...
def main():
    ...
    try:
        ...
        sys.exit(0 if runner.results["status"] == "SUCCESS" else 1)
