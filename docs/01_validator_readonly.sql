-- ==================================================================
-- UNIFIED VALIDATION SCRIPT - READ-ONLY OPERATIONS
-- ==================================================================
-- Usage: @validation/01_validator_readonly.sql <operation> <args...>
-- ==================================================================
...
-- ==================================================================
-- NOTE: This script does NOT modify data or schema (SELECT only)
-- For destructive operations, use 01_validator_write.sql
-- ==================================================================
SET SERVEROUTPUT ON
SET VERIFY OFF
...
PROMPT =============================================================
PROMPT UNIFIED VALIDATOR (READ-ONLY) - Operation: &operation
PROMPT =============================================================
...
BEGIN
    CASE UPPER('&operation')
        WHEN 'CHECK_SESSIONS' THEN
            -- Check for active sessions (READ-ONLY)
            FOR rec IN (
                SELECT COUNT(*) as cnt
        ...
        WHEN 'CHECK_TABLE_STRUCTURE' THEN
            -- Validate table structure (READ-ONLY)
            SELECT COUNT(*) INTO v_count
            FROM all_tables
            WHERE owner = UPPER('&owner') AND table_name = UPPER('&table_name');
        ...
        WHEN 'COUNT_ROWS' THEN
            -- Count rows with optional comparison (READ-ONLY)
        ...
        WHEN 'CHECK_CONSTRAINTS' THEN
            -- Check constraints status (READ-ONLY)
            DECLARE
            ...
            BEGIN
                ...
                IF v_action = 'CHECK' THEN
                    ...
                    END IF;
                ELSE
                    DBMS_OUTPUT.PUT_LINE('VALIDATION RESULT: ERROR - Enable/disable operations not available in read-only mode');
                    DBMS_OUTPUT.PUT_LINE('  Use 01_validator_write.sql for destructive operations');
                    v_result := FALSE;
                END IF;
            END;
            
        WHEN 'CHECK_PARTITION_DIST' THEN
            -- Show partition distribution (READ-ONLY)
            FOR rec IN (
