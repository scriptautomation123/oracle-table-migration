...
PROMPT ================================================================  
PROMPT Migration ID: MIG_JOB_QUEUE_20251025_210753
...
FROM APP_DATA_OWNER.JOB_QUEUE_NEW;

SELECT 'Data migration: ' || 
       CASE WHEN s.cnt = t.cnt THEN 'SUCCESS ✓' 
            ELSE 'FAILED ✗ (' || (s.cnt - t.cnt) || ' missing)' 
       END as step_20_result
FROM (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.JOB_QUEUE) s,
     (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.JOB_QUEUE_NEW) t;
...
EXEC DBMS_STATS.GATHER_TABLE_STATS('APP_DATA_OWNER', 'JOB_QUEUE', CASCADE => TRUE);
...
PROMPT Target: APP_DATA_OWNER.JOB_QUEUE (PartitionType.INTERVAL)
...
SELECT 
    CASE 
        ...
        WHEN EXISTS (SELECT 1 FROM user_part_tables WHERE table_name = 'JOB_QUEUE' AND partitioning_type = 'PartitionType.INTERVAL')
