...
class SubpartitionDefaults:
    ...
    size_based_recommendations: Dict[str, Any] = field(
        default_factory=dict
    )  # Size-based subpartition recommendations
...
class MigrationSettings:
    ...
    drop_old_after_days: int = (
        None  # Days to wait before dropping old table (0 = immediate)
    )
...
class TargetConfiguration:
    ...
    initial_partition_value: str = (
        None  # Initial partition boundary (Oracle TO_DATE format)
    )
    ...
    subpartition_count: int = (
        None  # Number of hash subpartitions (power of 2 recommended)
    )
...
class AvailableColumns:
    ...
    timestamp_columns: List[ColumnInfo] = field(
        default_factory=list
    )  # Timestamp/date columns suitable for interval partitioning
    numeric_columns: List[ColumnInfo] = field(
        default_factory=list
    )  # Numeric columns suitable for hash/range partitioning
    string_columns: List[ColumnInfo] = field(
        default_factory=list
    )  # String columns suitable for hash/list partitioning
...
class CurrentState:
    ...
    grants: List[Dict[str, Any]] = field(
        default_factory=list
    )  # Table grants and privileges for DDL generation
