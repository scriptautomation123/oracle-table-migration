...
class ConfigValidator:
    ...
    def validate_config(self, config: Dict, check_database: bool = False) -> Tuple[bool, List[str], List[str]]:
        """
        ...
        if check_database and self.connection:
            ...
            self.warnings.append("Database validation requested but no connection provided")
    ...
    def _validate_table_logic(self, table: Dict, index: int):
        ...
        if partition_col:
            ...
            if target_config.get('partition_type') == 'INTERVAL':
                self.errors.append(f"{prefix}: partition_column required for INTERVAL partitioning")
        ...
        if subpart_count > 0:
            if subpart_count > 1024:
                self.errors.append(f"{prefix}: subpartition_count {subpart_count} exceeds maximum (1024)")
        ...
        is_interval = current_state.get('is_interval', False)
        ...
        for table_config in config.get('tables', []):
            ...
            if partition_col:
                try:
                    ...
                    """, schema=schema, table_name=table_name, col_name=partition_col)
                    
                    result = cursor.fetchone()
                    if not result:
                        self.errors.append(f"{prefix}: partition column '{partition_col}' does not exist")
                    else:
                        data_type = result[0]
                        if data_type not in ['DATE', 'TIMESTAMP', 'TIMESTAMP(6)', 'TIMESTAMP(9)']:
                            self.warnings.append(
                                f"{prefix}: partition column '{partition_col}' type '{data_type}' may not be suitable for interval partitioning"
                            )
                except Exception as e:
                    self.errors.append(f"{prefix}: error checking partition column: {e}")
            ...
            if subpart_col:
                try:
                    cursor.execute("""
                        ...
                        WHERE owner = :schema AND table_name = :table_name
                          AND column_name = :col_name
                    """, schema=schema, table_name=table_name, col_name=subpart_col)
                    
                    result = cursor.fetchone()
                    if not result:
                        self.errors.append(f"{prefix}: subpartition column '{subpart_col}' does not exist")
                    else:
                    ...
                    self.errors.append(f"{prefix}: error checking subpartition column: {e}")
    ...
    @staticmethod
    def validate_file(config_file: str, connection=None, check_database: bool = False) -> bool:
        """
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Validate migration configuration file')
    ...
    if args.check_database and args.connection:
        try:
            ...
            try:
                ...
                print("ERROR: oracledb or cx_Oracle module required for database validation")
                exit(1)
