...
def get_template_filters():
    """Return dictionary of all custom filters for Jinja2"""
    def format_size_gb_filter(size_gb):
        """Format size in GB for display"""
        if not size_gb or size_gb <= 0:
            return "< 0.01 GB"
    ...
    return {
        'upper': lambda value: str(value).upper() if value else '',
        'lower': lambda value: str(value).lower() if value else '',
        'quote': lambda value: f"'{str(value)}'" if value is not None else 'NULL',
        'comma_separated': lambda value_list: ', '.join(str(item) for item in value_list) if value_list else '',
        'sql_identifier': lambda value: str(value).upper() if value else '',
        'format_interval': lambda interval_type, interval_value=1: f"INTERVAL '{interval_value}' {interval_type.upper()}" if interval_type else '',
        'yesno': lambda value, yes_text='YES', no_text='NO': yes_text if value else no_text,
        'format_size_gb': format_size_gb_filter,
        'format_row_count': format_row_count_filter,
        'estimate_time': estimate_time_filter,
        'parallel_hint': parallel_hint_filter,
        'match_condition': match_condition_filter,
        'format_column_list': format_column_list_filter
    }