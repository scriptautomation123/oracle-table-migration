...
class MigrationValidator:
    ...
    def validate_pre_migration(self, table_config: Dict) -> List[ValidationResult]:
        ...
        """
        self.logger.info(f"\n{'='*70}")
        self.logger.info(f"PRE-MIGRATION VALIDATION: {table_config['owner']}.{table_config['table_name']}")
        ...
        results.append(self._check_table_exists(
            table_config['owner'], 
        ...
        results.append(self._check_table_locks(
            table_config['owner'],
            table_config['table_name']
        ))
        
        # Check 6: Interval syntax valid
        if table_config.get('target_configuration', {}).get('partition_type') == 'INTERVAL':
    ...
    def validate_post_migration(self, table_config: Dict) -> List[ValidationResult]:
        ...
        """
        self.logger.info(f"\n{'='*70}")
        self.logger.info(f"POST-MIGRATION VALIDATION: {table_config['owner']}.{table_config['table_name']}")
        self.logger.info(f"{'='*70}")
        ...
        if table_config.get('target_configuration', {}).get('partition_type') == 'INTERVAL':
            results.append(self._check_interval_definition(table_config, new_table_name))
        
        # Check 4: Subpartition type correct
        if table_config.get('target_configuration', {}).get('subpartition_type'):
            results.append(self._check_subpartition_config(table_config, new_table_name))
    ...
    def compare_data(self, table_config: Dict) -> List[ValidationResult]:
        ...
        """
        self.logger.info(f"\n{'='*70}")
        self.logger.info(f"DATA COMPARISON: {table_config['owner']}.{table_config['table_name']}")
        self.logger.info(f"{'='*70}")
        ...
        results.append(self._compare_sample_data(owner, old_table, new_table, table_config))
        
        # Check 3: MIN/MAX values for partition column
        partition_col = table_config.get('target_configuration', {}).get('partition_column')
        if partition_col:
            results.append(self._compare_min_max_values(
                owner, old_table, new_table, partition_col
            ))
        
        # Check 4: Partition distribution
        if table_config.get('target_configuration', {}).get('partition_type') in ['RANGE', 'INTERVAL']:
    ...
    def _check_columns_exist(self, table_config: Dict) -> ValidationResult:
        ...
        sql = """
            ...
            WHERE owner = :owner 
              ...
              AND column_name IN ({})
        """.format(','.join([f"'{col}'" for col in columns_to_check]))
        ...
        try:
            ...
            if target_config.get('partition_type') == 'INTERVAL':
                ...
                if not any(data_type.startswith(vt) for vt in valid_types):
                    ...
                    result.success(f"Column {partition_col} type {data_type} is suitable")
            else:
    ...
    def _check_tablespace_space(self, table_config: Dict) -> ValidationResult:
        ...
        tablespace = table_config.get('target_configuration', {}).get('tablespace', 'USERS')
    ...
    def _check_interval_syntax(self, table_config: Dict) -> ValidationResult:
        ...
        elif interval_value < 1 or interval_value > 999:
            result.fail(
    ...
    def _check_partition_type(self, table_config: Dict, new_table_name: str) -> ValidationResult:
        """Verify partition type matches configuration"""
        result = ValidationResult("Partition Type")
        
        owner = table_config['owner']
        expected_type = table_config.get('target_configuration', {}).get('partition_type', 'RANGE')
        ...
        try:
            self.cursor.execute(sql, owner=owner.upper(), table_name=new_table_name.upper())
            row = self.cursor.fetchone()
    ...
    def _check_interval_definition(self, table_config: Dict, new_table_name: str) -> ValidationResult:
    ...
    def _check_subpartition_config(self, table_config: Dict, new_table_name: str) -> ValidationResult:
    ...
    def _check_row_counts(self, table_config: Dict, new_table_name: str) -> ValidationResult:
    ...
    def _check_indexes_created(self, table_config: Dict, new_table_name: str) -> ValidationResult:
    ...
    def _check_constraints(self, table_config: Dict, new_table_name: str) -> ValidationResult:
    ...
    def _compare_row_counts(self, owner: str, old_table: str, new_table: str) -> ValidationResult:
    ...
    def _compare_sample_data(self, owner: str, old_table: str, new_table: str, 
                            table_config: Dict) -> ValidationResult:
        ...
        try:
            self.cursor.execute(pk_sql, owner=owner.upper(), table_name=old_table.upper())
    ...
    def _compare_min_max_values(self, owner: str, old_table: str, 
                               new_table: str, column: str) -> ValidationResult:
        ...
        try:
            ...
            if old_min == new_min and old_max == new_max:
                ...
                result.fail(
                    ...
                    {
                        'old_min': str(old_min), 'old_max': str(old_max),
                        'new_min': str(new_min), 'new_max': str(new_max)
                    }
                )
    ...
    def _check_partition_distribution(self, owner: str, table_name: str) -> ValidationResult:
        ...
        try:
            ...
            if partitions:
                ...
                distribution = "\n".join([
                    f"  - {p[0]}: {p[1]:,} rows" if p[1] else f"  - {p[0]}: (no stats)"
...
"""
    ...
    def _format_results_table(self, results: List[ValidationResult]) -> str:
        ...
        for result in results:
            ...
            lines.append(f"| {icon} {result.status} | {result.check_name} | {message} |")
    ...
    def _generate_recommendations(self, results: List[ValidationResult]) -> str:
        ...
        if failed_count > 0:
            recommendations.append(f"- **CRITICAL:** {failed_count} check(s) failed. Do not proceed with migration until resolved.")
        
        if warning_count > 0:
            recommendations.append(f"- **CAUTION:** {warning_count} warning(s) found. Review before proceeding.")
        
        # Specific recommendations
        for result in results:
            if result.status == 'FAIL':
                if 'Row count mismatch' in result.message:
                    recommendations.append("- Investigate row count discrepancy before swapping tables")
                elif 'Partition type' in result.check_name:
                    recommendations.append("- Verify partition configuration in generated scripts")
        
        if not recommendations:
            recommendations.append("- ✓ All checks passed. Migration appears successful.")
