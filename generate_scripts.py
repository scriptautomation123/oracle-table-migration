...
class MigrationScriptGenerator:
    ...
    def discover_schema(self, schema_name: str, 
                       ...
                       output_file: str = 'migration_config.json') -> bool:
        """
        ...
        try:
            ...
            self.stats['tables_enabled'] = config['metadata']['tables_selected_for_migration']
            ...
            print(f"2. Customize settings:")
            print(f"   - Enable/disable tables (set 'enabled': true/false)")
            print(f"   - Choose partition column")
            print(f"   - Choose interval type (HOUR/DAY/WEEK/MONTH)")
            print(f"   - Choose hash subpartition column")
            print(f"   - Adjust hash subpartition count")
            print(f"3. Validate: python3 generate_scripts.py --config {output_file} --validate-only")
    ...
    def load_config(self) -> bool:
        ...
        try:
            ...
            self.stats['tables_enabled'] = metadata.get('tables_selected_for_migration', 0)
    ...
    def validate_config(self, check_database: bool = False) -> bool:
        ...
        try:
            ...
            is_valid, errors, warnings = validator.validate_config(
                self.config,
                check_database and connection is not None
            )
    ...
    def generate_validation_report(self, output_file: str = 'validation_report.md') -> bool:
        """
        ...
        try:
            ...
            # Run all validations
            for table_config in enabled_tables:
                print(f"Validating: {table_config.get('owner')}.{table_config.get('table_name')}")
            ...
            # Generate report
            report_path = validator.generate_report(output_file)
    ...
    def _prepare_template_context(self, table_config: Dict) -> Dict:
        ...
        timestamp_cols = [c['name'] for c in available_cols.get('timestamp_columns', [])]
        ...
        context = {
            ...
            'primary_key_columns': target_config.get('partition_column', all_columns[0] if all_columns else 'ID'),
    ...
    def _render_template(self, template_name: str, context: Dict, output_path: Path) -> bool:
        """
...
def main():
    ...
    parser.add_argument(
        '--schema', '-s',
        type=str,
        help='Schema name (required for discovery mode)'
    )
    parser.add_argument(
        '--include',
        type=str,
    ...
    if (args.validate_pre or args.validate_post or args.compare_data or args.validation_report) and not args.connection:
    ...
    if args.discover:
        ...
        if not generator.load_config():
            sys.exit(1)
        output_file = args.validation_report if args.validation_report != 'FILE' else 'validation_report.md'
