...
-- ==================================================================
-- Table: APP_DATA_OWNER.SESSION_DATA
...
DEFINE source_table = "SESSION_DATA"  
DEFINE target_table = "SESSION_DATA_NEW"

PROMPT ================================================================
PROMPT MASTER SCRIPT 1: COMPLETE END-TO-END MIGRATION
PROMPT ================================================================  
PROMPT Migration ID: MIG_SESSION_DATA_20251025_210754
PROMPT Source Table: APP_DATA_OWNER.SESSION_DATA
PROMPT Target Table: APP_DATA_OWNER.SESSION_DATA_NEW
PROMPT Migration Type: MigrationAction.ADD_INTERVAL_HASH_PARTITIONING
PROMPT ================================================================
PROMPT Current Size: 0.01 GB
PROMPT Current Rows: 30 rows
...
FROM all_tables WHERE owner = 'APP_DATA_OWNER' AND table_name = 'SESSION_DATA';

SELECT 'Source row count: ' || TO_CHAR(COUNT(*), '999,999,999,999') as validation  
FROM APP_DATA_OWNER.SESSION_DATA;
...
FROM APP_DATA_OWNER.SESSION_DATA_NEW;

SELECT 'Data migration: ' || 
       CASE WHEN s.cnt = t.cnt THEN 'SUCCESS ✓' 
            ELSE 'FAILED ✗ (' || (s.cnt - t.cnt) || ' missing)' 
       END as step_20_result
FROM (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.SESSION_DATA) s,
     (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.SESSION_DATA_NEW) t;
...
WHERE i.owner = 'APP_DATA_OWNER' AND i.table_name = 'SESSION_DATA_NEW'
...
AND table_name IN ('SESSION_DATA', 'SESSION_DATA_NEW')
...
PROMPT ================================================================
PROMPT Old table: APP_DATA_OWNER.SESSION_DATA_OLD
...
EXEC DBMS_STATS.GATHER_TABLE_STATS('APP_DATA_OWNER', 'SESSION_DATA', CASCADE => TRUE);
...
FROM APP_DATA_OWNER.SESSION_DATA_OLD;
...
FROM (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.SESSION_DATA_OLD) o,
     (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.SESSION_DATA) n;
...
EXPLAIN PLAN FOR 
SELECT COUNT(*) FROM APP_DATA_OWNER.SESSION_DATA 
...
PROMPT ================================================================
PROMPT Migration Type: MigrationAction.ADD_INTERVAL_HASH_PARTITIONING
PROMPT Source: APP_DATA_OWNER.SESSION_DATA (Non-partitioned)
PROMPT Target: APP_DATA_OWNER.SESSION_DATA (PartitionType.INTERVAL)
PROMPT Data Migrated: 30 rows rows
PROMPT Size: 0.01 GB
...
SELECT 
    CASE 
        WHEN EXISTS (SELECT 1 FROM user_constraints WHERE table_name = 'SESSION_DATA' AND status = 'DISABLED')
        THEN 'WARNING: Some constraints still disabled ⚠️'
        WHEN NOT EXISTS (SELECT 1 FROM user_tables WHERE table_name = 'SESSION_DATA')  
        THEN 'ERROR: Target table missing ✗'
        WHEN EXISTS (SELECT 1 FROM user_part_tables WHERE table_name = 'SESSION_DATA' AND partitioning_type = 'PartitionType.INTERVAL')
