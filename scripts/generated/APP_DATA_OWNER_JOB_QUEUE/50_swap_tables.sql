...
-- ==================================================================
-- SWAP TABLES: APP_DATA_OWNER.JOB_QUEUE <-> APP_DATA_OWNER.JOB_QUEUE_NEW
-- ==================================================================
...
PROMPT ================================================================
PROMPT Old table: APP_DATA_OWNER.JOB_QUEUE
PROMPT New table: APP_DATA_OWNER.JOB_QUEUE_NEW
PROMPT Backup: APP_DATA_OWNER.JOB_QUEUE_OLD
PROMPT ================================================================
...
BEGIN
    ...
    IF v_original_exists = 0 THEN
        RAISE_APPLICATION_ERROR(-20001, 'Original table APP_DATA_OWNER.JOB_QUEUE not found');
    END IF;
    ...
    IF v_new_exists = 0 THEN
        RAISE_APPLICATION_ERROR(-20002, 'New table APP_DATA_OWNER.JOB_QUEUE_NEW not found');
    END IF;
    ...
    DBMS_OUTPUT.PUT_LINE('Step 1: Renaming JOB_QUEUE → JOB_QUEUE_OLD');
    BEGIN
        EXECUTE IMMEDIATE 'ALTER TABLE APP_DATA_OWNER.JOB_QUEUE RENAME TO JOB_QUEUE_OLD';
        DBMS_OUTPUT.PUT_LINE('✓ Renamed JOB_QUEUE to JOB_QUEUE_OLD');
    EXCEPTION
        WHEN OTHERS THEN
    ...
    DBMS_OUTPUT.PUT_LINE('Step 2: Renaming JOB_QUEUE_NEW → JOB_QUEUE');
    BEGIN
        EXECUTE IMMEDIATE 'ALTER TABLE APP_DATA_OWNER.JOB_QUEUE_NEW RENAME TO JOB_QUEUE';
        DBMS_OUTPUT.PUT_LINE('✓ Renamed JOB_QUEUE_NEW to JOB_QUEUE');
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            BEGIN
                EXECUTE IMMEDIATE 'ALTER TABLE APP_DATA_OWNER.JOB_QUEUE_OLD RENAME TO JOB_QUEUE';
                DBMS_OUTPUT.PUT_LINE('✓ Rollback successful: JOB_QUEUE_OLD restored to JOB_QUEUE');
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    ...
                    DBMS_OUTPUT.PUT_LINE('  - Original table is now: APP_DATA_OWNER.JOB_QUEUE_OLD');
                    DBMS_OUTPUT.PUT_LINE('  - New table is still: APP_DATA_OWNER.JOB_QUEUE_NEW');
                    DBMS_OUTPUT.PUT_LINE('  - Expected table name JOB_QUEUE is unavailable');
    ...
    SELECT COUNT(*) INTO v_original_exists
    FROM all_tables 
    WHERE owner = 'APP_DATA_OWNER' AND table_name = 'JOB_QUEUE_OLD';
    ...
    IF v_original_exists = 1 AND v_new_exists = 1 THEN
        ...
        DBMS_OUTPUT.PUT_LINE('  - Old table: APP_DATA_OWNER.JOB_QUEUE_OLD ✓');
        DBMS_OUTPUT.PUT_LINE('  - Active table: APP_DATA_OWNER.JOB_QUEUE ✓');
    ELSE
    ...
    FOR invalid_obj IN (
        ...
        AND object_name IN ('JOB_QUEUE', 'JOB_QUEUE_OLD')
    ) LOOP
...
@validation/verify_table_states.sql APP_DATA_OWNER JOB_QUEUE JOB_QUEUE_OLD
