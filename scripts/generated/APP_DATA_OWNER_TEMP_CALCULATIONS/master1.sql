...
DEFINE source_table = "TEMP_CALCULATIONS"  
DEFINE target_table = "TEMP_CALCULATIONS_NEW"

PROMPT ================================================================
PROMPT MASTER SCRIPT 1: COMPLETE END-TO-END MIGRATION
PROMPT ================================================================  
PROMPT Migration ID: MIG_TEMP_CALCULATIONS_20251025_210754
PROMPT Source Table: APP_DATA_OWNER.TEMP_CALCULATIONS
PROMPT Target Table: APP_DATA_OWNER.TEMP_CALCULATIONS_NEW
PROMPT Migration Type: MigrationAction.ADD_INTERVAL_HASH_PARTITIONING
PROMPT ================================================================
PROMPT Current Size: 0.01 GB
PROMPT Current Rows: 5 rows
...
FROM all_tables WHERE owner = 'APP_DATA_OWNER' AND table_name = 'TEMP_CALCULATIONS';
...
WHERE table_name = 'TEMP_CALCULATIONS' 
AND status = 'DISABLED';
...
FROM APP_DATA_OWNER.TEMP_CALCULATIONS_NEW;

SELECT 'Data migration: ' || 
       CASE WHEN s.cnt = t.cnt THEN 'SUCCESS ✓' 
            ELSE 'FAILED ✗ (' || (s.cnt - t.cnt) || ' missing)' 
       END as step_20_result
FROM (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.TEMP_CALCULATIONS) s,
     (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.TEMP_CALCULATIONS_NEW) t;
...
WHERE i.owner = 'APP_DATA_OWNER' AND i.table_name = 'TEMP_CALCULATIONS_NEW'
...
AND table_name IN ('TEMP_CALCULATIONS', 'TEMP_CALCULATIONS_NEW')
...
PROMPT ================================================================
PROMPT Old table: APP_DATA_OWNER.TEMP_CALCULATIONS_OLD
...
EXEC DBMS_STATS.GATHER_TABLE_STATS('APP_DATA_OWNER', 'TEMP_CALCULATIONS', CASCADE => TRUE);
...
FROM APP_DATA_OWNER.TEMP_CALCULATIONS_OLD;
...
FROM (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.TEMP_CALCULATIONS_OLD) o,
     (SELECT COUNT(*) cnt FROM APP_DATA_OWNER.TEMP_CALCULATIONS) n;
...
EXPLAIN PLAN FOR 
SELECT COUNT(*) FROM APP_DATA_OWNER.TEMP_CALCULATIONS 
WHERE CALC_TIMESTAMP >= SYSDATE - 30;
...
PROMPT ================================================================
PROMPT Migration Type: MigrationAction.ADD_INTERVAL_HASH_PARTITIONING
PROMPT Source: APP_DATA_OWNER.TEMP_CALCULATIONS (Non-partitioned)
PROMPT Target: APP_DATA_OWNER.TEMP_CALCULATIONS (PartitionType.INTERVAL)
PROMPT Data Migrated: 5 rows rows
PROMPT Size: 0.01 GB
...
SELECT 
    CASE 
        WHEN EXISTS (SELECT 1 FROM user_constraints WHERE table_name = 'TEMP_CALCULATIONS' AND status = 'DISABLED')
        THEN 'WARNING: Some constraints still disabled ⚠️'
        WHEN NOT EXISTS (SELECT 1 FROM user_tables WHERE table_name = 'TEMP_CALCULATIONS')  
        THEN 'ERROR: Target table missing ✗'
        WHEN EXISTS (SELECT 1 FROM user_part_tables WHERE table_name = 'TEMP_CALCULATIONS' AND partitioning_type = 'PartitionType.INTERVAL')
