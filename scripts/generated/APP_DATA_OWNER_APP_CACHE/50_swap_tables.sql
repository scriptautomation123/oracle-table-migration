...
-- ==================================================================
-- SWAP TABLES: APP_DATA_OWNER.APP_CACHE <-> APP_DATA_OWNER.APP_CACHE_NEW
-- ==================================================================
...
PROMPT ================================================================
PROMPT Old table: APP_DATA_OWNER.APP_CACHE
PROMPT New table: APP_DATA_OWNER.APP_CACHE_NEW
PROMPT Backup: APP_DATA_OWNER.APP_CACHE_OLD
PROMPT ================================================================
...
BEGIN
    ...
    IF v_original_exists = 0 THEN
        RAISE_APPLICATION_ERROR(-20001, 'Original table APP_DATA_OWNER.APP_CACHE not found');
    END IF;
    ...
    IF v_new_exists = 0 THEN
        RAISE_APPLICATION_ERROR(-20002, 'New table APP_DATA_OWNER.APP_CACHE_NEW not found');
    END IF;
    ...
    DBMS_OUTPUT.PUT_LINE('Step 1: Renaming APP_CACHE → APP_CACHE_OLD');
    BEGIN
        EXECUTE IMMEDIATE 'ALTER TABLE APP_DATA_OWNER.APP_CACHE RENAME TO APP_CACHE_OLD';
        DBMS_OUTPUT.PUT_LINE('✓ Renamed APP_CACHE to APP_CACHE_OLD');
    EXCEPTION
        WHEN OTHERS THEN
    ...
    DBMS_OUTPUT.PUT_LINE('Step 2: Renaming APP_CACHE_NEW → APP_CACHE');
    BEGIN
        EXECUTE IMMEDIATE 'ALTER TABLE APP_DATA_OWNER.APP_CACHE_NEW RENAME TO APP_CACHE';
        DBMS_OUTPUT.PUT_LINE('✓ Renamed APP_CACHE_NEW to APP_CACHE');
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            BEGIN
                EXECUTE IMMEDIATE 'ALTER TABLE APP_DATA_OWNER.APP_CACHE_OLD RENAME TO APP_CACHE';
                DBMS_OUTPUT.PUT_LINE('✓ Rollback successful: APP_CACHE_OLD restored to APP_CACHE');
            ...
            EXCEPTION
                WHEN OTHERS THEN
                    ...
                    DBMS_OUTPUT.PUT_LINE('  - Original table is now: APP_DATA_OWNER.APP_CACHE_OLD');
                    DBMS_OUTPUT.PUT_LINE('  - New table is still: APP_DATA_OWNER.APP_CACHE_NEW');
                    DBMS_OUTPUT.PUT_LINE('  - Expected table name APP_CACHE is unavailable');
    ...
    SELECT COUNT(*) INTO v_original_exists
    FROM all_tables 
    WHERE owner = 'APP_DATA_OWNER' AND table_name = 'APP_CACHE_OLD';
    ...
    IF v_original_exists = 1 AND v_new_exists = 1 THEN
        ...
        DBMS_OUTPUT.PUT_LINE('  - Old table: APP_DATA_OWNER.APP_CACHE_OLD ✓');
        DBMS_OUTPUT.PUT_LINE('  - Active table: APP_DATA_OWNER.APP_CACHE ✓');
    ELSE
    ...
    FOR invalid_obj IN (
        ...
        AND object_name IN ('APP_CACHE', 'APP_CACHE_OLD')
    ) LOOP
...
@validation/verify_table_states.sql APP_DATA_OWNER APP_CACHE APP_CACHE_OLD
