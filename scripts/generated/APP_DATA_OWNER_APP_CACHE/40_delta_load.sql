...
-- ==================================================================
-- DELTA LOAD: APP_DATA_OWNER.APP_CACHE_NEW
-- ==================================================================
...
DECLARE
    v_partition_column VARCHAR2(128) := 'EXPIRY_TIME';
    v_delta_interval VARCHAR2(20) := 'last_day';
    v_cutoff_timestamp DATE;
    v_table_name VARCHAR2(128) := 'APP_DATA_OWNER.APP_CACHE';
    v_sql VARCHAR2(4000);
BEGIN
...
MERGE /*+ PARALLEL(4) */ INTO APP_DATA_OWNER.APP_CACHE_NEW tgt
USING (
    SELECT EXPIRY_TIME, CACHE_KEY
...
ON (tgt.EXPIRY_TIME = src.EXPIRY_TIME)
WHEN MATCHED THEN
    UPDATE SET 
WHEN NOT MATCHED THEN
    INSERT (EXPIRY_TIME, CACHE_KEY)
    VALUES (EXPIRY_TIME, CACHE_KEY);
